			...
			<div v-if="showGagarin && !showGuideModal && !gameStarted">
				<div class="fixed sm:hidden lt-sm:flex bottom-0 flex w-full left-0 right-0">
					...
					<div  class="absolute bottom-180px lt-md:bottom-80px right-100px flex lt-sm:flex-col items-center min-w-238px h-94px justify-between gap-y-7px py-10px px-20px relative border-2px border-#C9C9C9 bg-white ">
						...
						<button  :class="easyMode ? ['w-167px'] : ['w-176px']"  @click="showGuideModal = true; showGagarin = false" class="py-12px lt-md:py-4px lt-sm:h-40px xxxl:text-20px lt-xxl:text-14px lt-xl:text-14px bg-#71CB8A leading-140% q-shadow-sm__lightest-green text-white">{{ easyMode ? 'Не изи, а легкий' : 'Не хард, а сложный' }}</button>
					</div>
				</div>
				<div class="fixed left-40% lt-sm:hidden lt-sm:left-unset lt-sm:right-30px lt-sm:translate-x-unset lt-xxl:left-33% translate-x-[-50%] bottom-0 flex">
					...
					<div class="flex lt-sm:flex-col items-center justify-between gap-x-26px py-20px lt-lg:py-14px px-34px relative border-2px border-#C9C9C9 h-100px lt-lg:h-76px bg-white -ml-100px lt-sm:m-0! xxxl:min-w-570px lt-xxxl:min-w-520px lt-xxl:min-w-380px lt-xl:min-w-450px lt-lg:absolute lt-lg:bottom-330px lt-lg:ml-40% lt-lg:w-440px lt-lg:gap-x-5 lt-sm:max-w-230px! lt-sm:w-230px! lt-sm:min-w-230px!">
						...
						<button @click="showGuideModal = true; showGagarin = false" :class="easyMode ? ['w-320px'] : ['w-344px']" class="h-52px p-12px xxxl:text-18px lt-xxl:text-14px lt-xl:text-14px bg-#71CB8A w-280px lt-xxl:w-185px leading-140% q-shadow-sm__lightest-green text-white">{{ easyMode ? 'Не изи, а легкий' : 'Не хард, а сложный' }}</button>
					</div>
				</div>
			</div>
        </Transition>
