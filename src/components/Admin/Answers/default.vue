<template>
    <div class="answers-default">
        ...
        <UIButton @click="handleSaveOrUpdateAnswer(state)">Сохранить Ответы</UIButton>
    </div>
</template>
...
const handleSaveOrUpdateAnswer = async (answer) =>{
    for (let index = 0; index < answer.length; index++) {
        if (answer[index].id === 'new') {
            await saveAnswer({ ...answer[index], id: undefined }).then(data => {
                if (data) state.value[index] = data
            })
        } else {
            await updateAnswer({ ...answer[index], updated: undefined }).then(data => {
                if (data) state.value[index] = data
            })
        }
    }
