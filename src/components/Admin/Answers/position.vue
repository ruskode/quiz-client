<template>
  <div class="answers-position flex" style="flex-direction: row">
    <div class="answers-position_content zx">
      ...
      <div class="answers-position_item" v-for="(answer, idx) in state" :key="`answer--position__${idx}`">
        ...
        <el-input @change="startChangeAnswer(answer, idx)" v-model="answer.meta.title" :rows="1" placeholder="Ответ" />
        <p>{{ answer.correctPos }}</p>
      </div>
    </div>
    <div class="answers-position_content zx">
      <p class="answers-position__title">Позиции</p>
      <div class="answers-position_item" v-for="(position, idx) in questionState?.meta?.positions" :key="`answer--position__${idx}`">
        <p @click="handleDeletePosition(position, idx)" class="answers-position_item__delete">x</p>
        <el-input @change="startUpdatePosition(position, idx)" v-model="position.title" :rows="1" placeholder="Название позиции" />
        <el-input @change="startUpdatePosition(position, idx)" style="max-width: 30px" v-model="position.position" />
      </div>
    </div>
  </div>
  <UIButton @click="handleAddAnswerPosition()">Добавить ответ</UIButton>
  <UIButton @click="handleSaveOrUpdatePosition()">Сохранить Ответы</UIButton>
</template>
...
const handleSaveOrUpdatePosition = async () => {
  await updateQeustion({
    id: props.question.id,
    meta: {
      ...questionState.value.meta,
      positions: questionState.value.meta.positions,
    },
  }).then((data) => {
  ...
  state.value.forEach(async (item) => {
    if (item.id === 'new') {
      await saveAnswer({ ...item, id: undefined }).then((data) => {
        if (data) item = data
      })
    } else {
      await updateAnswer({ ...item, updated: undefined }).then((data) => {
        if (data) item = data
      })
    }
  })
