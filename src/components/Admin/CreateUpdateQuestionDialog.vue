<template>
	<div class="create-update-dialog dialog">
		<div class="dialog_header">
			<p @click="handleUpdateImage()" class="dialog_header__delete-image">
				X
			</p>
			<img v-if="getValidImage.src" class="dialog_header__image" style="width: 100%;"
				:src="getValidImage.src ? getValidImage.src : props.image" />
			<div class="dialog_header_upload w-min-100%" v-else>
				<UIInputfiles
					class="w-min-100%"
					@change="hasNewImage = true"
					v-model="state.meta.image"
					@error="(data) => (imageError = data)"
				/>
				<div class="dialog_header_upload_content">
					...
					<p v-if="imageError" style="color: red">
						Изображение неверного размера!
					</p>
				</div>
			</div>
		</div>
		<div class="dialog_content">
			...
			<div
				...
				<UIRadioButtons v-model="state.type" @change="(value) => (state.type = value?.id)"
					style="width: fit-content;" :options="typeOptions" />
				<span>Легкий Вопрос? <input type="checkbox" v-model="state.meta.easy"> </span>
			</div>
			<UIButton @click="handleUpdateCreateQuestion(), $emit('close')">{{ createMode ? 'Создать' : 'Сохранить вопрос'
			}} </UIButton>
...
const stateAnswers = computed(() => {
	if(state?.value?.id) return getAnswersById(state.value.id)
	return []
})
const state = ref({
	id: undefined,
	...
	if (props.createMode) {
		await saveQuestion(state.value).then((data) => {
			if (data) {
				...
				// todo: chto za pizdec pochemu foto net
			} else {
