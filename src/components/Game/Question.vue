...
<template>
	<div class="flex flex-col w-screen items-center max-h-90vh relative z-1">
		...
		<div
			v-if="question.meta && question.meta.easy"
			class="flex items-center scale-90 sm:hidden w-full justify-between"
			:class="{ 'pointer-events-none!': answerEmitting }"
		>
			<div
				@click="goToStart"
		...
		>
			<div
				v-if="question.meta && !question.meta.easy"
				class="flex lt-sm:hidden justify-between"
			>
				<div class="timer bg-white lt-xxxl:h-55px! lt-xxxl:max-w-310px q-shadow-sm lt-xxxl:p-10px!">
					<img
						class="w-24px h-24px lt-xxxl:w-18px lt-xxxl:h-18px"
			...
			>
				<div
					@click="goToStart"
					class="timer bg-white lt-xxxl:h-55px! q-shadow-sm p-25px! max-w-98px! h-82px! cursor-pointer relative pb-6px"
				>
					<img
						src="/img/logo.svg"
						class="w-46px h-30px"
				...
				>
					<div class="flex flex-col mt-30px mb-30px lt-2xl:mb-20px lt-sm:mt-0!">
						...
						<p
							...
							class="text-36px lt-sm:text-18px lt-md:text-22px font-700 leading-130% mt-16px"
						></p>
						<div class="relative">
							<img
								v-if="question.meta.image && question.type !== 'POSITION'"
								class="w-full h-256px lt-sm:h-136px lt-md:h-168px object-cover q-shadow-sm mt-24px lt-sm:mt-14px"
						...
						<p
							...
							class="font-500 text-26px lt-sm:text-17px leading-130% text-#1E2947 mt-38px lt-sm:mt-28px"
						></p>
						<p
							v-else
							class="font-500 text-26px lt-md:text-18px lt-sm:text-14px leading-130% text-#1E2947 mt-26px lt-sm:mt-18px lg:hidden"
						>
							Выберите ответ
						</p>
					</div>
					...
					>
						<div
							...
							class="q-answer q-shadow-sm flex items-center justify-center max-h-70px lt-sm:max-h-7vh overflow-hidden lt-md:w-full! lt-md:max-h-5.5vh"
							:class="[
						...
						>
							<p
								...
								class="text-20px lt-xxxl:text-14px leading-130% text-center py-22px p-18px lt-sm:px-12px "
							></p>
						</div>
					</div>
					<div
					...
					<div
						...
						class="flex flex-col lt-lg:flex-row flex-wrap lt-lg:gap-x-8px lt-sm:w-95%"
					>
						<div class="flex flex-wrap gap-16px lt-lg:flex-col lt-lg:w-[calc(100%/2-4px)]">
							<div
								v-for="(position, index) in question.meta.positions"
								class="bg-white q-answer q-shadow-sm w-[calc(100%/3-12px)]! lt-lg:w-full! lt-lg:max-h-60px lt-sm:max-h-38px flex items-center"
							>
								<p class="text-20px lt-xxxl:text-18px lt-sm:text-14px leading-26px text-center py-22px px-18px lt-sm:px-10px lt-sm:py-14px w-full pointer-events-none">
									{{ position.title }}
								</p>
							</div>
						</div>
				...
				<div class="absolute bottom-0 left-50% translate-x-[-50%] z-1010101001">
					...
					>
						<div class="translate-y--10px">
							<p
								class="text-#08882C font-500 text-36px sm:text-30px lt-lg:text-30px leading-130%"
